...
export type DispatchPressureDecision =
  | {
      status: "allow";
      reason: "below_threshold" | "cgroup_unavailable" | "cgroup_unbounded";
  ...
  | {
      status: "defer";
      reason: "cgroup_memory_threshold" | "cgroup_memory_growth";
      sample: DispatchPressureSample;
...
): DispatchPressureSample | undefined | "unbounded" {
...
function withGrowth(
  ...
  previousSample = { ts: now, currentBytes: sample.currentBytes, maxBytes: sample.maxBytes };
  ...
  const growthBytes = sample.currentBytes - previous.currentBytes;
  return growthBytes > 0
    ? {
        ...sample,
        previousCurrentBytes: previous.currentBytes,
        growthBytes,
        windowMs,
      }
    : sample;
...
export function decideDispatchPressure(
  ...
  if (rawSample === "unbounded") {
    return input.override
      ? { status: "override", reason: "chris_approved_urgent", override: input.override }
      : { status: "allow", reason: "cgroup_unbounded" };
  }
  ...
  const growthBytesLimit = Math.min(
  ...
  const thresholdDecision =
    sample.usageRatio >= usageRatioLimit
      ? {
          ...
          sample,
          threshold: {
            currentBytes: Math.floor(sample.maxBytes * usageRatioLimit),
